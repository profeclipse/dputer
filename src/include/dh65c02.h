...
public:
  ...
  inline void resetStatus() { PS = uF; }

  inline void setFlag(uint8_t flag, bool value) {
    PS = value ? PS | flag : PS & ~flag;
  }

  inline bool getFlag(uint8_t flag) { return PS & flag; }
  ...
  inline uint8_t read(uint16_t addr) { return bus->read(addr); }

  inline uint16_t readWord(uint16_t addr) { return bus->readWord(addr); }

  inline void write(uint16_t addr, uint8_t value) { bus->write(addr, value); }
