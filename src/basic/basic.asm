...
LAB_WARM 	= USER_ZP		; BASIC warm start entry point
Wrmjpl 		= USER_ZP+1	; BASIC warm start vector jump low byte
Wrmjph 		= USER_ZP+2	; BASIC warm start vector jump high byte

Usrjmp		= USER_ZP+3		; USR function JMP address
Usrjpl		= USER_ZP+4		; USR function JMP vector low byte
Usrjph		= USER_ZP+5		; USR function JMP vector high byte
Nullct		= USER_ZP+6		; nulls output after each line
TPos		= USER_ZP+7		; BASIC terminal position byte
TWidth		= USER_ZP+8		; BASIC terminal width byte
Iclim		= USER_ZP+9		; input column limit
Itempl		= USER_ZP+10	; temporary integer low byte
Itemph		= USER_ZP+11	; temporary integer high byte

nums_1		= USER_ZP+10	; number to bin/hex string convert MSB
nums_2		= USER_ZP+11	; number to bin/hex string convert
nums_3		= USER_ZP+12	; number to bin/hex string convert LSB

Srchc		= USER_ZP+13	; search character
Temp3		= Srchc			; temp byte used in number routines
Scnquo		= USER_ZP+14	; scan-between-quotes flag
...
Ibptr		= USER_ZP+15	; input buffer pointer
Dimcnt		= Ibptr			; # of dimensions
Tindx		= Ibptr			; token index

Defdim		= USER_ZP+16	; default DIM flag
Dtypef		= USER_ZP+17	; data type flag, $FF=string, $00=numeric
Oquote		= USER_ZP+18	; open quote flag (b7) (Flag: DATA scan; LIST quote; memory)
Gclctd		= USER_ZP+19	; garbage collected flag
Sufnxf		= USER_ZP+20	; subscript/FNX flag, 1xxx xxx = FN(0xxx xxx)
Imode		= USER_ZP+21	; input mode flag, $00=INPUT, $80=READ

Cflag		= USER_ZP+22	; comparison evaluation flag

TabSiz		= USER_ZP+23	; TAB step size (was input flag)

next_s		= USER_ZP+24	; next descriptor stack address

							; these two bytes form a word pointer to the item
							; currently on top of the descriptor stack
last_sl		= USER_ZP+25	; last descriptor stack address low byte
last_sh		= USER_ZP+26	; last descriptor stack address high byte (always $00)

des_sk		= USER_ZP+27	; descriptor stack start address (temp strings)

;			= USER_ZP+36	; End of descriptor stack

ut1_pl		= USER_ZP+37	; utility pointer 1 low byte
ut1_ph		= USER_ZP+38	; utility pointer 1 high byte
ut2_pl		= USER_ZP+39	; utility pointer 2 low byte
ut2_ph		= USER_ZP+40	; utility pointer 2 high byte

Temp_2		= ut1_pl		; temp byte for block move	

FACt_1		= USER_ZP+41	; FAC temp mantissa1
FACt_2		= USER_ZP+42	; FAC temp mantissa2
FACt_3		= USER_ZP+43	; FAC temp mantissa3

dims_l		= FACt_2		; array dimension size low byte
dims_h		= FACt_3		; array dimension size high byte

TempB		= USER_ZP+44	; temp page 0 byte

Smeml		= USER_ZP+45	; start of mem low byte		(Start-of-Basic)
Smemh		= USER_ZP+46	; start of mem high byte	(Start-of-Basic)
Svarl		= USER_ZP+47	; start of vars low byte	(Start-of-Variables)
Svarh		= USER_ZP+48	; start of vars high byte	(Start-of-Variables)
Sarryl		= USER_ZP+49	; var mem end low byte		(Start-of-Arrays)
Sarryh		= USER_ZP+50	; var mem end high byte		(Start-of-Arrays)
Earryl		= USER_ZP+51	; array mem end low byte	(End-of-Arrays)
Earryh		= USER_ZP+52	; array mem end high byte	(End-of-Arrays)
Sstorl		= USER_ZP+53	; string storage low byte	(String storage (moving down))
Sstorh		= USER_ZP+54	; string storage high byte	(String storage (moving down))
Sutill		= USER_ZP+55	; string utility ptr low byte
Sutilh		= USER_ZP+56	; string utility ptr high byte
Ememl		= USER_ZP+57	; end of mem low byte		(Limit-of-memory)
Ememh		= USER_ZP+58	; end of mem high byte		(Limit-of-memory)
Clinel		= USER_ZP+59	; current line low byte		(Basic line number)
Clineh		= USER_ZP+60	; current line high byte	(Basic line number)
Blinel		= USER_ZP+61	; break line low byte		(Previous Basic line number)
Blineh		= USER_ZP+62	; break line high byte		(Previous Basic line number)

Cpntrl		= USER_ZP+63	; continue pointer low byte
Cpntrh		= USER_ZP+64	; continue pointer high byte

Dlinel		= USER_ZP+65	; current DATA line low byte
Dlineh		= USER_ZP+66	; current DATA line high byte

Dptrl		= USER_ZP+67	; DATA pointer low byte
Dptrh		= USER_ZP+68	; DATA pointer high byte

Rdptrl		= USER_ZP+69	; read pointer low byte
Rdptrh		= USER_ZP+70	; read pointer high byte

Varnm1		= USER_ZP+71	; current var name 1st byte
Varnm2		= USER_ZP+72	; current var name 2nd byte

Cvaral		= USER_ZP+73	; current var address low byte
Cvarah		= USER_ZP+74	; current var address high byte

Frnxtl		= USER_ZP+75	; var pointer for FOR/NEXT low byte
Frnxth		= USER_ZP+76	; var pointer for FOR/NEXT high byte
...
prstk		= USER_ZP+77	; precedence stacked flag

comp_f		= USER_ZP+79	; compare function flag, bits 0,1 and 2 used
							; bit 2 set if >
							; bit 1 set if =
							; bit 0 set if <

func_l		= USER_ZP+80	; function pointer low byte
func_h		= USER_ZP+81	; function pointer high byte

garb_l		= func_l		; garbage collection working pointer low byte
garb_h		= func_h		; garbage collection working pointer high byte

des_2l		= USER_ZP+82	; string descriptor_2 pointer low byte
des_2h		= USER_ZP+83	; string descriptor_2 pointer high byte

g_step		= USER_ZP+84	; garbage collect step size

Fnxjmp		= USER_ZP+85	; jump vector for functions
Fnxjpl		= USER_ZP+86	; functions jump vector low byte
Fnxjph		= USER_ZP+87	; functions jump vector high byte

g_indx		= Fnxjpl		; garbage collect temp index

FAC2_r		= USER_ZP+88	; FAC2 rounding byte

Adatal		= USER_ZP+89	; array data pointer low byte
Adatah		= USER_ZP+90	; array data pointer high  byte

Nbendl		= Adatal		; new block end pointer low byte
Nbendh		= Adatah		; new block end pointer high  byte

Obendl		= USER_ZP+91	; old block end pointer low byte
Obendh		= USER_ZP+92	; old block end pointer high  byte

numexp		= USER_ZP+92	; string to float number exponent count
expcnt		= USER_ZP+93	; string to float exponent count

numbit		= numexp		; bit count for array element calculations

numdpf		= USER_ZP+94	; string to float decimal point flag
expneg		= USER_ZP+95	; string to float eval exponent -ve flag
...
FAC1_e		= USER_ZP+96	; FAC1 exponent
FAC1_1		= USER_ZP+97	; FAC1 mantissa1
FAC1_2		= USER_ZP+98	; FAC1 mantissa2
FAC1_3		= USER_ZP+99	; FAC1 mantissa3
FAC1_s		= USER_ZP+100	; FAC1 sign (b7)
...
negnum		= USER_ZP+101 	; string to float eval -ve flag
numcon		= USER_ZP+101	; series evaluation constant count

FAC1_o		= USER_ZP+102	; FAC1 overflow byte

FAC2_e		= USER_ZP+103	; FAC2 exponent
FAC2_1		= USER_ZP+104	; FAC2 mantissa1
FAC2_2		= USER_ZP+105	; FAC2 mantissa2
FAC2_3		= USER_ZP+106	; FAC2 mantissa3
FAC2_s		= USER_ZP+107	; FAC2 sign (b7)

FAC_sc		= USER_ZP+108	; FAC sign comparison, Acc#1 vs #2
FAC1_r		= USER_ZP+109	; FAC1 rounding byte
...
csidx		= USER_ZP+110	; line crunch save index
Asptl		= csidx			; array size/pointer low byte
Aspth		= USER_ZP+111	; array size/pointer high byte
...
LAB_IGBY	= USER_ZP+112	; get next BASIC byte subroutine

LAB_GBYT	= USER_ZP+118	; get current BASIC byte subroutine
Bpntrl		= USER_ZP+119	; BASIC execute (get byte) pointer low byte
Bpntrh		= USER_ZP+120	; BASIC execute (get byte) pointer high byte

;			= USER_ZP+139	; end of get BASIC char subroutine

Rbyte4		= USER_ZP+140	; extra PRNG byte
Rbyte1		= USER_ZP+141	; most significant PRNG byte
Rbyte2		= USER_ZP+142	; middle PRNG byte
Rbyte3		= USER_ZP+143	; least significant PRNG byte

NmiBase		= USER_ZP+144	; NMI handler enabled/setup/triggered flags
							; bit	function
							; ===	========
							; 7	interrupt enabled
							; 6	interrupt setup
...
IrqBase		= USER_ZP+147	; IRQ handler enabled/setup/triggered flags
...
LAB_MSZM
	.text	$0A,"Memory size ",$00

LAB_SMSG
	.text	" Bytes free",$0A; ,$0A
	.text	"Enhanced BASIC 2.22",$0A,$00
...
LAB_BMSG	.text	$0A,"Break",$00
LAB_EMSG	.text	" Error",$00
LAB_LMSG	.text	" in line ",$00
LAB_RMSG	.text	$0A,"Ready",$0A,$00

LAB_IMSG	.text	" Extra ignored",$0A,$00
LAB_REDO	.text	" Redo from start",$0A,$00
